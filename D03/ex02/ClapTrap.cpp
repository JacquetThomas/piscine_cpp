...
/*   Updated: 2019/03/28 16:50:14 by cjacquet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void        ClapTrap::rangedAttack(std::string const & target) {
    std::cout << "CL4P-TP " << this->_name << " attacks " << target << " at range, causing " << this->_rangedAttackDamage << " damages" << std::endl;
}

void        ClapTrap::meleeAttack(std::string const & target) {
    std::cout << "CL4P-TP " << this->_name << " attacks " << target << " at melee, causing " << this->_meleeAttackDamage << " damages" << std::endl;
}
...
void        ClapTrap::takeDamage(unsigned int amount) {
    ...
    if (this->_hitPoints == 0) {
        std::cout << "CL4P-TP " << this->_name << " Stop it! i'm already dead !" << std::endl;
        return ;
    }
    damage = amount - this->getArmorDamageReduction();
    std::cout << "CL4P-TP " << this->_name << " takes " << damage << " damages" << std::endl;
    if (damage >= this->getHitPoints()) {
        this->setHitPoints(0);
        std::cout << "CL4P-TP " << this->_name << " Aaaannnnnd I'm dead." << std::endl;
    } else {
...
void        ClapTrap::beRepaired(unsigned int amount) {
    if (amount > this->_energyPoints) {
        std::cout << "CL4P-TP " << this->_name << " Not enough energy !" << std::endl;
